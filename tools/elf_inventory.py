...
def format_symbol_file(
    ...
    for symbol in sorted(
        ...
        if symbol.symbol_type == Elf32SymbolTableType.FUNC.value:
            ...
            if symbol.address in defined_addresses:
                attributes.append("defined:true")
        if symbol.size:
...
def build_outputs() -> dict[Path, str]:
    ...
    defined_addresses = source_function_addresses()
    ...
    return {
        SYMBOLS_PATH: format_symbol_file(symbols, defined_addresses),
