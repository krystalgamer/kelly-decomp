...
def patch_ee_stack_saves(path: Path) -> None:
    ...
    for offset in range(text_offset, text_offset + text_size - 84, 4):
        words = list(struct.unpack_from("<22I", data, offset))
        if words == [
            0x27BDFFB0, 0x0000382D, 0x7FB20020, 0x00C0902D,
            0x7FB30030, 0x7FB10010, 0x0080982D, 0x00A0882D,
            0x7FB00000, 0x3C06004F, 0x0100802D, 0x7FBF0040,
            0x24C686F0, 0x240402D0, 0x0C0AB15E, 0x0000282D,
            0x0220282D, 0x0200302D, 0x0000402D, 0x0040202D,
            0x0C0B3D58, 0x24070005,
        ]:
            words[3:5] = (words[4], words[3])
            words[7:9] = (words[8], words[7])
            words[10:12] = (words[11], words[10])
            words[16:22] = (
                words[17], words[18], words[21],
                words[19], words[20], words[16],
            )
            struct.pack_into("<22I", data, offset, *words)

    for offset in range(text_offset, text_offset + text_size - 40, 4):
        words = list(struct.unpack_from("<11I", data, offset))
        if words == [
            0x27BDFFA0, 0x7FB40040, 0x7FB30030, 0x00C0A02D,
            0x7FB20020, 0x00A0982D, 0x7FB10010, 0x0080902D,
            0x00E0882D, 0x7FB00000, 0x7FBF0050,
        ]:
            words[8], words[9] = words[9], words[8]
            struct.pack_into("<11I", data, offset, *words)

    for offset in range(text_offset, text_offset + text_size - 12, 4):
        words = list(struct.unpack_from("<4I", data, offset))
        if words == [
            0x00021027,
            0x0002280B,
            0x0481FFFA,
            ...
            struct.pack_into("<4I", data, offset, *words)

    for offset in range(text_offset, text_offset + text_size - 180, 4):
        words = list(struct.unpack_from("<46I", data, offset))
        if words == [
            0x27BDFF80, 0x7FB20050, 0x7FB10040, 0x00A0902D,
            0x7FB00030, 0x00C0882D, 0xE7B50078, 0x0080802D,
            0xE7B40070, 0x46006546, 0x7FBF0060, 0x46006D06,
            0x8E040010, 0x8C820000, 0xAFA40010, 0x10440016,
            0xAFA20000, 0x00000000, 0x8FA30000, 0x8C670008,
            0x8CE20014, 0x30420001, 0x1040000B, 0x0240282D,
            0x8CE20140, 0x0220302D, 0x4600AB06, 0x84440050,
            0x4600A346, 0x8C430054, 0x0060F809, 0x00E42021,
            0x8E040010, 0x8FA30000, 0x8C620000, 0xAFA40020,
            0x1444FFED, 0xAFA20000, 0x7BBF0060, 0x7BB20050,
            0x7BB10040, 0x7BB00030, 0xC7B50078, 0xC7B40070,
            0x03E00008, 0x27BD0080,
        ]:
            words[:] = [
                0x27BDFF90, 0x7FB20040, 0x7FB10030, 0x00A0902D,
                0x7FB00020, 0x00C0882D, 0xE7B50068, 0x0080802D,
                0xE7B40060, 0x46006546, 0x7FBF0050, 0x46006D06,
                0x8E020010, 0x8C430000, 0xAFA20010, 0x10620016,
                0xAFA30000, 0x00000000, 0x8FA30000, 0x8C670008,
                0x8CE20014, 0x30420001, 0x10400009, 0x0240282D,
                0x8CE30140, 0x0220302D, 0x4600AB06, 0x84640050,
                0x4600A346, 0x8C620054, 0x0040F809, 0x00E42021,
                0x8FA20000, 0x8E040010, 0x8C430000, 0xAFA40010,
                0x1464FFED, 0xAFA30000, 0x7BBF0050, 0x7BB20040,
                0x7BB10030, 0x7BB00020, 0xC7B50068, 0xC7B40060,
                0x03E00008, 0x27BD0070,
            ]
            struct.pack_into("<46I", data, offset, *words)

    for offset in range(text_offset, text_offset + text_size - 72, 4):
        words = list(struct.unpack_from("<19I", data, offset))
        if words == [
            0x3C02003E, 0xAE00000C, 0x24435628, 0xAE000010,
            0xAE000014, 0xA3A00010, 0x8C620008, 0x14400005,
            0x27B20010, 0x0C0436B8, 0x24040018, 0x10000004,
            0x0040202D, 0x0040202D, 0x8C820000, 0xAC620008,
            0xAE240000, 0x0200102D, 0xAE200004,
        ]:
            words[2] = 0x24455628
            words[6] = 0x8CA30008
            words[7] = 0x14600005
            words[13:16] = (0x8C620000, 0x0060202D, 0xACA20008)
            struct.pack_into("<19I", data, offset, *words)
