# OnMenuOpen__26MenuEntryFunctionFloatEditP4MenuP10MenuSystem
...
The released function float edit menu-open handler runs the base hook before fixing its value.

`KELLY_DECOMP_COMPILER_BARRIER()` is a matching-only annotation that emits no target instruction. It prevents EE GCC from applying the sibling/tail-call or scheduling transformation described above.

## Outcome

The released `MenuEntryFunctionFloatEdit::OnMenuOpen` implementation matched exactly.
