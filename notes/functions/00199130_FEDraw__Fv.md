...
All three released C++ forms load `frontendmanager` and tail-call `FEManager::DrawFE` in 12 bytes. The target retains a 32-byte frame, so the wrapper was deferred.
