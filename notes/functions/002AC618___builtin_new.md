...
| 3 | different | 27.2727 | 18.1818 | `candidate.cpp` |
...
The released global allocator forwards to `mem_malloc` with the `operator new` description and zero line/flags. A matching-only register constraint emits no instruction; it keeps the absolute page base live so EE GCC schedules the RA save between the target's high- and low-half address operations.

`KELLY_DECOMP_COMPILER_BARRIER()` is a matching-only annotation that emits no target instruction. It prevents EE GCC from applying the sibling/tail-call or scheduling transformation described above.

## Outcome

The released global `operator new` implementation matched exactly.
