# PopFront__23IGOLearnNewTrickManagerb

- Address: `0x0015B418`
- Size: `0x5C` (92 bytes)
- Object: `game/files_frontend`
- Debug source: `C:/KS/SRC/ks/igolearn_new_trickmanager.cpp`
- Reference source: `KS/SRC/ks/igolearn_new_trickmanager.cpp`
- Result: **matched**

## Attempts

| # | Status | Byte score | Instruction score | Candidate |
| ---: | --- | ---: | ---: | --- |
| 1 | matched | 100.0 | 100.0 | `candidate.cpp` |

### Attempt 1 notes

Read the released PopFront flow and exact manager layout, then reconstructed the binary-version completion timer reset using released globals. It matched byte-exactly on attempt 1.

## Outcome

The existing implementation already uses the shared learn-new-trick manager
declaration and remains exact.
