# start_secondary_cam__22kellyslater_controllerP6camera

- Address: `0x0021F670`
- Size: `0x6C` (108 bytes)
- Object: `game/files_kellyslater`
- Debug source: `C:/KS/SRC/ks/kellyslater_controller.cpp`
- Reference source: `KS/SRC/ks/kellyslater_controller.cpp`
- Result: **matched**

## Attempts

| # | Status | Byte score | Instruction score | Candidate |
| ---: | --- | ---: | ---: | --- |
| 1 | matched | 100.0 | 100.0 | `candidate.cpp` |

### Attempt 1 notes

Read all blocked state/super-state values, photo/look-back camera offsets, and SetPlayerCamera call. It matched on attempt 1.

## Outcome

Canonicalized with shared released declarations and natural C++; exact target bytes preserved.
