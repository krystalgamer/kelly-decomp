...
All three shared-declaration forms load `target`, adjust to the beam, and tail-call `beam::set_thickness` in 12 bytes. The target retains a 32-byte frame, so the wrapper was deferred.
