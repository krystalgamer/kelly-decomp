...
Used the exact released destructor body; release_buffer matched but GCC tail-called conditional delete.
...
Tested a virtual declaration after the diff; the added vptr store and larger body diverged.

## Outcome

Deferred after five source-only attempts; release_buffer behavior matches, but isolated GCC tail-calls delete while the target retains a normal call.
