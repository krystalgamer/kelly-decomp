...
As with the adjacent start and A queries, the released body emits the correct 68-byte logic but omits the target's FPU hazard `nop` after `mtc1`. Five source-only forms failed to preserve that instruction, so the function was deferred rather than forcing target bytes.
