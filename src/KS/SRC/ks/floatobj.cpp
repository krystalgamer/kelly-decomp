...
bool floating_object::parse_params(char **arguments,int count)
{
    bool result;
    if (water_object::parse_params(arguments,count)) {
        read_float_param(
            arguments,count,water_text,&water_interaction);
        result=true;
    } else {
        result=false;
    }
