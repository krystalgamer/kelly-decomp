// Matching decompilation blocks selected by generated build shims.


// 0x00328440 __cl__31slf_signaller_disable_signals_tR8vm_stackQ320script_library_class8function7entry_t
#include "KS/SRC/script_lib_signal.h"

bool slf_signaller_disable_signals_t::operator()(vm_stack &stack, entry_t entry) { SLF_PARMS; parms->me->disable(); SLF_DONE; }

// 0x003283A8 __cl__30slf_signaller_enable_signals_tR8vm_stackQ320script_library_class8function7entry_t
#include "KS/SRC/script_lib_signal.h"

bool slf_signaller_enable_signals_t::operator()(
    vm_stack &stack,
    entry_t entry)
{
    SLF_PARMS;
    parms->me->enable();
    SLF_DONE;
}

// 0x003284D8 __cl__31slf_signaller_clear_callbacks_tR8vm_stackQ320script_library_class8function7entry_t
#include "KS/SRC/script_lib_signal.h"

__asm__(".equ clear_script_callbacks__9signaller, 0x0034CDD8");

bool slf_signaller_clear_callbacks_t::operator()(
    vm_stack &stack,
    entry_t entry
) {
    SLF_PARMS;
    parms->me->clear_script_callbacks();
    SLF_DONE;
}

#if defined(KELLY_DECOMP_FUNCTION_00328578)
// 0x00328578 __cl__30slf_signaller_clear_callback_tR8vm_stackQ320script_library_class8function7entry_t
#include "KS/SRC/script_lib_signal.h"

__asm__(".equ clear_script_callback__9signallerRC7stringx, 0x0034CE48");

bool slf_signaller_clear_callback_t::operator()(vm_stack &stack, entry_t entry) {
    SLF_PARMS;
    parms->me->clear_script_callback(*parms->name);
    SLF_DONE;
}
#endif

#if defined(KELLY_DECOMP_FUNCTION_00328358)
// 0x00328358 __tf30slf_signaller_enable_signals_t
#include "KS/SRC/rtti.h"

extern "C" void **RttiBase_00328358()
    __asm__("__tfQ220script_library_class8function");
extern "C" void *rtti_type_00328358[]
    __asm__("__ti30slf_signaller_enable_signals_t");
extern "C" const char rtti_name_00328358[];
extern "C" void *rtti_base_type_00328358[]
    __asm__("__tiQ220script_library_class8function");
__asm__(".equ __tfQ220script_library_class8function, 0x0035F640");
__asm__(".equ __ti30slf_signaller_enable_signals_t, 0x005A4A90");
__asm__(".equ rtti_name_00328358, 0x0050F708");
__asm__(".equ __tiQ220script_library_class8function, 0x005121B8");
extern "C" void **Rtti_00328358() __asm__("__tf30slf_signaller_enable_signals_t");

void **Rtti_00328358()
{
    if (!rtti_type_00328358[0]) {
        RttiBase_00328358();
        __rtti_si(rtti_type_00328358, rtti_name_00328358, rtti_base_type_00328358);
    }
    return rtti_type_00328358;
}
#endif

#if defined(KELLY_DECOMP_FUNCTION_003283F0)
// 0x003283F0 __tf31slf_signaller_disable_signals_t
#include "KS/SRC/rtti.h"

extern "C" void **RttiBase_003283F0()
    __asm__("__tfQ220script_library_class8function");
extern "C" void *rtti_type_003283F0[]
    __asm__("__ti31slf_signaller_disable_signals_t");
extern "C" const char rtti_name_003283F0[];
extern "C" void *rtti_base_type_003283F0[]
    __asm__("__tiQ220script_library_class8function");
__asm__(".equ __tfQ220script_library_class8function, 0x0035F640");
__asm__(".equ __ti31slf_signaller_disable_signals_t, 0x005A4AA0");
__asm__(".equ rtti_name_003283F0, 0x0050F730");
__asm__(".equ __tiQ220script_library_class8function, 0x005121B8");
extern "C" void **Rtti_003283F0() __asm__("__tf31slf_signaller_disable_signals_t");

void **Rtti_003283F0()
{
    if (!rtti_type_003283F0[0]) {
        RttiBase_003283F0();
        __rtti_si(rtti_type_003283F0, rtti_name_003283F0, rtti_base_type_003283F0);
    }
    return rtti_type_003283F0;
}
#endif

#if defined(KELLY_DECOMP_FUNCTION_00328488)
// 0x00328488 __tf31slf_signaller_clear_callbacks_t
#include "KS/SRC/rtti.h"

extern "C" void **RttiBase_00328488()
    __asm__("__tfQ220script_library_class8function");
extern "C" void *rtti_type_00328488[]
    __asm__("__ti31slf_signaller_clear_callbacks_t");
extern "C" const char rtti_name_00328488[];
extern "C" void *rtti_base_type_00328488[]
    __asm__("__tiQ220script_library_class8function");
__asm__(".equ __tfQ220script_library_class8function, 0x0035F640");
__asm__(".equ __ti31slf_signaller_clear_callbacks_t, 0x005A4AB0");
__asm__(".equ rtti_name_00328488, 0x0050F758");
__asm__(".equ __tiQ220script_library_class8function, 0x005121B8");
extern "C" void **Rtti_00328488() __asm__("__tf31slf_signaller_clear_callbacks_t");

void **Rtti_00328488()
{
    if (!rtti_type_00328488[0]) {
        RttiBase_00328488();
        __rtti_si(rtti_type_00328488, rtti_name_00328488, rtti_base_type_00328488);
    }
    return rtti_type_00328488;
}
#endif

#if defined(KELLY_DECOMP_FUNCTION_00328528)
// 0x00328528 __tf30slf_signaller_clear_callback_t
#include "KS/SRC/rtti.h"

extern "C" void **RttiBase_00328528()
    __asm__("__tfQ220script_library_class8function");
extern "C" void *rtti_type_00328528[]
    __asm__("__ti30slf_signaller_clear_callback_t");
extern "C" const char rtti_name_00328528[];
extern "C" void *rtti_base_type_00328528[]
    __asm__("__tiQ220script_library_class8function");
__asm__(".equ __tfQ220script_library_class8function, 0x0035F640");
__asm__(".equ __ti30slf_signaller_clear_callback_t, 0x005A4AC0");
__asm__(".equ rtti_name_00328528, 0x0050F780");
__asm__(".equ __tiQ220script_library_class8function, 0x005121B8");
extern "C" void **Rtti_00328528() __asm__("__tf30slf_signaller_clear_callback_t");

void **Rtti_00328528()
{
    if (!rtti_type_00328528[0]) {
        RttiBase_00328528();
        __rtti_si(rtti_type_00328528, rtti_name_00328528, rtti_base_type_00328528);
    }
    return rtti_type_00328528;
}
#endif
