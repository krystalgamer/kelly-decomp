...
int mem_get_largest_avail(int heap) { return mem_raw_largest_avail(heap) - 128; }
#endif
...
extern "C" void check_leaks(int checkpoint, unsigned int flags)
    __asm__("mem_check_leaks_since_checkpoint__FiUi");

void mem_leak_test(bool strict)
{
    mem_check_heap();
    if (leak_checkpoint > 0) {
        void (*check)(int, unsigned int) = check_leaks;
        check(leak_checkpoint, strict ? 7u : 3u);
    }
}
#endif
...
int mem_get_total_alloced(int heapindex) { return heaps[heapindex].GetTotalUsed(); }
#endif

#if defined(KELLY_DECOMP_FUNCTION_002ACB20)
// 0x002ACB20 mem_get_total_mem__Fi
#include "KS/SRC/archalloc.h"

int mem_get_total_mem(int heapindex) { return heaps[heapindex].GetTotalSize(); }
#endif
...
int mem_get_total_avail(int heapindex) { return heaps[heapindex].GetTotalFree(); }
#endif
