...
#include "KS/SRC/archalloc.h"
#include "KS/SRC/hw_texture_ref.h"
...
extern "C" typeface_def::inter_kern *UninitializedCopy(
    typeface_def::inter_kern *first,
    typeface_def::inter_kern *last,
    typeface_def::inter_kern *result
) __asm__("__uninitialized_copy_aux__H2ZPQ212typeface_def10inter_kernZPQ212typeface_def10inter_kern_X01X01X11G12__false_type_X11");
typeface_def::inter_kern *UninitializedCopy(
    typeface_def::inter_kern *first,
    typeface_def::inter_kern *last,
    typeface_def::inter_kern *result
)
{
    while (first != last) {
        if (result) new (result) typeface_def::inter_kern(*first);
        ++first;
        ++result;
    }
    return result;
...
#include "KS/SRC/game_control_tree.h"

extern "C" game_control_iterator *tree_lower_bound(
    game_control_iterator *result,
...
game_control_iterator *tree_lower_bound(
    game_control_iterator *result,
    game_control_tree *tree,
    const int *key)
{
    game_control_node *candidate = tree->header;
    game_control_node *current = tree->header->parent;
    while (current != 0) {
        if (!(current->key < *key)) {
            candidate = current;
